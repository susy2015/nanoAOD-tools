...
class OutputBranch:
    def __init__(self, tree, name, rootBranchType, n=1, lenVar=None, title=None, limitedPrecision=None, saveBranch=True):
        n = int(n)
        self.buff   = array(_rootBranchType2PythonArray[rootBranchType], n*[0. if rootBranchType in 'FD' else 0])
        self.lenVar = lenVar
        self.n = n
        self.precision = ROOT.ReduceMantissaToNbitsRounding(int(limitedPrecision)) if limitedPrecision and rootBranchType=='F' else lambda x : x
...
class OutputTree:
    def __init__(self, tfile, ttree, intree, branchSelection):
        self._file = tfile
        self._tree = ttree
        self._intree = intree
        self._branchSelection = branchSelection
        self._branches = {} 
    def branch(self, name, rootBranchType, n=1, lenVar=None, title=None, limitedPrecision=None):
...
class FullOutput(OutputTree):
    def __init__(
        ...
        OutputTree.__init__(self, outputFile, outputTree, inputTree, outputbranchSelection)
